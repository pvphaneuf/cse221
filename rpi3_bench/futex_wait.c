...
int main(void) {
    ...
    *futex = 0xA;
    ...
    if (child_pid == CHILD_PID) {
        ...
        while(1) {
            syscall(SYS_futex, futex, FUTEX_WAKE, 1, NULL, NULL, 42);
        }
    }
    ...
    for (unsigned int i = 0; i < TEST_COUNT_PER_PROCESS; i++) {
        syscall(SYS_futex, futex, FUTEX_WAIT, 0xA, NULL, NULL, 42);  // Assume that a successful and unsuccessful FUTEX_WAIT results in the same amount of time.
    }
    ...
    printf("%i FUTEX_WAIT system calls in %lluns (%.1fns per FUTEX_WAIT system calls)\n",
